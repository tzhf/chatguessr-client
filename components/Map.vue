    ...
    if (props.bot) {
        if (props.user) {
            ...
            L.popup({ closeButton: false })
                .setLatLng([0, 0])
                .setContent("<b>Click on the map to make your guess !</b>")
                .openOn(map);
        }
    }
    ...
    map.on("click", (e: any) => {
        ...
        if (props.user) {
            ...
            L.popup({ closeButton: false })
                .setLatLng(e.latlng)
                .setContent(
                    `${command}<br><span class="font-bold text-lg">Copied to clipboard</span><br><small>Paste (ctrl+v) without editing</small>`
                )
                .openOn(map);
        }
    });
